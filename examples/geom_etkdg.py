...
class Worker(Process):
    ...
    def run(self):
        for mol_id in iter(self.queue.get, None):
            pred_mol = Chem.rdmolfiles.MolFromPDBFile(os.path.join(base_path, "conformer-rl/examples/data/geom_etkdg/", f"mol_{mol_id}.pdb"), removeHs=False)
            ref_mol = Chem.rdmolfiles.MolFromPDBFile(os.path.join(base_path, "conformer-rl/examples/data/geom_truth/", f"{mol_id}.pdb"), removeHs=False)
            pred_mol = utils.prune_conformers(pred_mol, tfd_thresh=0.05)
            
            pruned_dir = os.path.join(base_path, "conformer-rl/examples/data/geom_etkdg_pruned/")
